...
// TODO: Sever DriveConstants imports. Do not use imports, we have to pass these in.
...
public class SwerveUtils {
    ...
    public static Rotation2d removeAzimuthJitter(SwerveModuleState setpoint, SwerveModuleState current) {
        return Math.abs(setpoint.speedMetersPerSecond / kMaxLinearSpeedMPS) < kJitterThreshold
                ? current.angle
                : setpoint.angle;
    }
    ...
    public static double optimizeTorque(
            SwerveModuleState unOptimized, SwerveModuleState optimized, double motorAmperage, int i) {
    ...
    public static boolean isSpeedOptimized(SwerveModuleState state, SwerveModuleState optimizedState, int i) {
        boolean isSpeedOptimized =
                !EqualsUtil.epsilonEquals(state.speedMetersPerSecond, optimizedState.speedMetersPerSecond);
    ...
    public static double convertChoreoNewtonsToAmps(SwerveModuleState currentState, DriveFeedforwards ff, int i) {
        double choreoLinearForceNewtons = projectTorque(
                currentState,
                VecBuilder.fill(ff.robotRelativeForcesXNewtons()[i], ff.robotRelativeForcesYNewtons()[i]));
    ...
    public static void logPossibleDriveStates(
            boolean doLogging,
        ...
        if (doLogging) {
            ...
            Telemetry.log(
                    "Drive/Odometry/preOptimizedChassisSpeeds", kKinematics.toChassisSpeeds(unOptimizedSetpointStates));
            ...
            Telemetry.log(
                    "Drive/Odometry/FieldRegularChassisSpeeds",
