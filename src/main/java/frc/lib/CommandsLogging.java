...
public class CommandsLogging {
    ...
    public static void logRunningCommands() {
        ...
        Logger.recordOutput("CommandScheduler/Running/warnings", runningCommands.toArray(new String[0]));
        Logger.recordOutput("CommandScheduler/Running/infos", runningDefaultCommands.toArray(new String[0]));
    ...
    private static void addCommand(Command command, String parentCommandName, ArrayList<String> commandsList) {
        if (command instanceof WrapperCommand wrapperCommand) {
            // Grab the command being wrapped by the WrapperCommand
            try {
                addCommand((Command) wrapperCommandField.get(wrapperCommand), parentCommandName, commandsList);
