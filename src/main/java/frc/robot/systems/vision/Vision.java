...
public class Vision {
    ...
    private static final LoggedTunableNumber kSingleXYStdev =
            new LoggedTunableNumber("Vision/kSingleXYStdev", kSingleStdDevs.get(0));
    private static final LoggedTunableNumber kMultiXYStdev =
            new LoggedTunableNumber("Vision/kMultiXYStdev", kMultiStdDevs.get(0));

    public static final AprilTagFieldLayout k2025Field =
    ...
    public void periodic(Pose2d lastRobotPose, Pose2d simOdomPose) {
        for (int i = 0; i < cameras.length; i++) {
            ...
            // Logger.recordOutput("Vision/"+camerasData[i].camName+"/Pose",
            // camerasData[i].latestEstimatedRobotPose.toPose2d());
            // Logger.recordOutput("Vision/"+camerasData[i].camName+"/X",
            // camerasData[i].latestEstimatedRobotPose.getRotation().getX());
            // Logger.recordOutput("Vision/"+camerasData[i].camName+"/Y",
            // camerasData[i].latestEstimatedRobotPose.getRotation().getY());
            // Logger.recordOutput("Vision/"+camerasData[i].camName+"/Z",
            // camerasData[i].latestEstimatedRobotPose.getRotation().getZ());
        }
    }
            ...
            if (camData.hasTarget && camData.hasBeenUpdated) {
                ...
                for (int r = 0; r < camData.latestTagTransforms.length; r++) {
                    if (camData.latestTagTransforms[r] != null) {
                        if (camData.latestTagAmbiguities[r] < kAmbiguityThreshold) {
                            avgDistMeters += camData.latestTagTransforms[r]
                                    .getTranslation()
                                    .getNorm();
                        } else {
                            numberOfTargets -= 1;
                ...
                if (numberOfTargets == 1 && avgDistMeters > 3.5) {
                    ...
                    observations[i] = new VisionObservation(
                            ...
                            singleTagPose,
                            VecBuilder.fill(
                                    kSingleXYStdev.get() * xyScalar, kSingleXYStdev.get() * xyScalar, Double.MAX_VALUE),
                            camData.latestTimestamp,
                            camData.camName);
                    ...
                    observations[i] = new VisionObservation(
                            ...
                            camData.latestEstimatedRobotPose.toPose2d(),
                            VecBuilder.fill(
                                    kMultiXYStdev.get() * xyScalar, kMultiXYStdev.get() * xyScalar, Double.MAX_VALUE),
                            camData.latestTimestamp,
                            camData.camName);
                }
            } else {
